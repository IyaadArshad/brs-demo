...
export async function POST(request: Request) {
  ...
  async function FetchId(name: string): Promise<Response> {
    try {
      const record = await pb.collection("files")
        .getFirstListItem(`file_name='${name}'`, { fields: "id" });
  ...
  const existingRecord = await pb.collection("files").getOne(fetchIdData.id);
