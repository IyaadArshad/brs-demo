    ...
    let conversation: Message[] = [
      {
        role: "system",
        content:
          'You are ChatGPT, but modified to work as a Business Requirements Specification (BRS) AI Agent. CRITICAL: When using the implement_edits function, you must pass the user\'s original message exactly as received in the user_inputs parameter. DO NOT format, modify, or pre-process the message. DO NOT try to write the content yourself. Example - if user says "Add a test screen", pass exactly that, not your interpretation of what the screen should look like. Let the implementation handlers deal with interpreting the user\'s request. ' +
